                    ...
                    if value > 0 and value <= constants.ROLE_VIEWER and (value & value - 1 == 0 or value == 1):
                        if vals["value"] == "true":
                            ...
                            if (
                                ...
                                .filter(
                                    ub.User.role.op("&")(constants.ROLE_ADMIN) == constants.ROLE_ADMIN,  # pyright: ignore[reportAttributeAccessIssue]
                                    ub.User.id != user.id,
                                )
                                .count()
                                        ...
                                        [
                                            {
                                                "type": "danger",
                                                "message": _(
                                                    "No admin user remaining, can't remove admin role", nick=user.name
                                                ),
                                            }
                                        ]
                                    )
...
def edit_restriction(res_type, user_id):
    ...
    restriction_names = {
        0: "allowed_tags",
        1: "allowed_column_values",
        2: "per_user_allowed_tags",
        3: "per_user_allowed_columns",
        4: "denied_tags",
        5: "denied_column_values",
        6: "per_user_denied_tags",
        7: "per_user_denied_columns",
    }
...
def add_restriction(res_type, user_id):
    element = request.form.to_dict()
    restriction_types = {
        0: "allowed_tags_template",
        1: "allowed_column_values_template",
        2: "per_user_tags",
        3: "per_user_column_values",
    }
...
def delete_restriction(res_type, user_id):
    element = request.form.to_dict()
    restriction_types = {
        0: "denied_tags_template",
        1: "denied_column_values_template",
        2: "per_user_tags",
        3: "per_user_column_values",
    }
    res_name = restriction_types.get(res_type, "unknown_restriction")
    ...
    if to_save.get("config_use_google_drive"):
        ...
        if "config_use_google_drive" in to_save and not config.config_use_google_drive and not gdrive_error:
            ...
            gdriveutils.update_settings(
                gdrive_secrets["client_id"], gdrive_secrets["client_secret"], gdrive_secrets["redirect_uris"][0]
            )
...
def _configuration_ldap_helper(to_save):
    ...
    if (config.config_ldap_cacert_path or config.config_ldap_cert_path or config.config_ldap_key_path) and not (
        ...
        return reboot_required, _configuration_result(
            _("LDAP CACertificate, Certificate or Key Location is not Valid, Please Enter Correct Path")
        )
    return reboot_required, None
...
def _configuration_update_helper():
    ...
    try:
        ...
        reboot_required |= (
            _config_checkbox_int(to_save, "config_anonbrowse") and config.config_login_type == constants.LOGIN_LDAP
        )
