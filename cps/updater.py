        ...
        if request_method == "GET":
            ...
            while i >= 0:
                ...
                if major_version_update == current_version[0]:
                    if (
                        minor_version_update == current_version[1] and patch_version_update > current_version[2]  # pyright: ignore[reportOperatorIssue]
                    ) or minor_version_update > current_version[1]:  # pyright: ignore[reportOperatorIssue]
                        parents.append([commit[i]["tag_name"], commit[i]["body"].replace("\r\n", "<p>")])
                        newer = True
                    i -= 1
                    continue
