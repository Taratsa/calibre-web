...
from . import calibre_db, config, constants, db, isoLanguages, logger, ub
from .helper import get_book_cover, get_download_link
from .pagination import Pagination
from .usermanagement import auth, requires_basic_auth_if_no_ano
...
@requires_basic_auth_if_no_ano
def feed_booksindex():
    return render_element_index(db.Books.sort, None, 'opds.feed_letter_books')  # pyright: ignore[reportGeneralTypeIssues]
...
def feed_letter_books(book_id):
    ...
    letter = true() if book_id == "00" else func.upper(db.Books.sort).startswith(book_id)  # pyright: ignore[reportGeneralTypeIssues]
...
@requires_basic_auth_if_no_ano
def feed_new():
    if not auth.current_user().check_visibility(constants.SIDEBAR_RECENT):  # pyright: ignore[reportOptionalMemberAccess]
        abort(404)
    off = request.args.get("offset") or 0
    entries, __, pagination = calibre_db.fill_indexpage((int(off) / (int(config.config_books_per_page)) + 1), 0,
                                                        db.Books, True, [db.Books.timestamp.desc()],  # pyright: ignore[reportGeneralTypeIssues]
                                                        True, config.config_read_column)
    cc = calibre_db.get_cc_columns(config, filter_config_custom_read=True)
    return render_xml_template('feed.xml', entries=entries, pagination=pagination, cc=cc)
...
@requires_basic_auth_if_no_ano
def feed_discover():
    if not auth.current_user().check_visibility(constants.SIDEBAR_RANDOM):  # pyright: ignore[reportOptionalMemberAccess]
        abort(404)
...
@requires_basic_auth_if_no_ano
def feed_best_rated():
    if not auth.current_user().check_visibility(constants.SIDEBAR_BEST_RATED):  # pyright: ignore[reportOptionalMemberAccess]
        abort(404)
    off = request.args.get("offset") or 0
    entries, __, pagination = calibre_db.fill_indexpage((int(off) / (int(config.config_books_per_page)) + 1), 0,
                                                        db.Books, db.Books.ratings.any(db.Ratings.rating > 9),  # pyright: ignore[reportGeneralTypeIssues]
...
@requires_basic_auth_if_no_ano
def feed_hot():
    if not auth.current_user().check_visibility(constants.SIDEBAR_HOT):  # pyright: ignore[reportOptionalMemberAccess]
        abort(404)
    off = request.args.get("offset") or 0
    all_books = ub.session.query(ub.Downloads, func.count(ub.Downloads.book_id)).order_by(  # pyright: ignore[reportArgumentType]
        func.count(ub.Downloads.book_id).desc()).group_by(ub.Downloads.book_id)  # pyright: ignore[reportArgumentType]
...
def feed_authorindex():
    if not auth.current_user().check_visibility(constants.SIDEBAR_AUTHOR):  # pyright: ignore[reportOptionalMemberAccess]
        abort(404)
    return render_element_index(db.Authors.sort, db.books_authors_link, 'opds.feed_letter_author')  # pyright: ignore[reportGeneralTypeIssues]
...
def feed_letter_author(book_id):
    if not auth.current_user().check_visibility(constants.SIDEBAR_AUTHOR):  # pyright: ignore[reportOptionalMemberAccess]
        abort(404)
    off = request.args.get("offset") or 0
    letter = true() if book_id == "00" else func.upper(db.Authors.sort).startswith(book_id)  # pyright: ignore[reportGeneralTypeIssues]
    entries = calibre_db.session.query(db.Authors).join(db.books_authors_link).join(db.Books)\
        ...
        .order_by(db.Authors.sort)  # pyright: ignore[reportGeneralTypeIssues]
    pagination = Pagination((int(off) / (int(config.config_books_per_page)) + 1), config.config_books_per_page,
                            entries.count())
    entries = entries.limit(config.config_books_per_page).offset(off).all()  # pyright: ignore[reportArgumentType]
    cc = calibre_db.get_cc_columns(config, filter_config_custom_read=True)
...
def feed_publisherindex():
    if not auth.current_user().check_visibility(constants.SIDEBAR_PUBLISHER):  # pyright: ignore[reportOptionalMemberAccess]
        abort(404)
    off = request.args.get("offset") or 0
    entries = (calibre_db.session.query(db.Publishers)
        ...
        .order_by(db.Publishers.sort)  # pyright: ignore[reportGeneralTypeIssues]
        .limit(config.config_books_per_page).offset(off))  # pyright: ignore[reportArgumentType]
    pagination = Pagination((int(off) / (int(config.config_books_per_page)) + 1), config.config_books_per_page,
...
def feed_categoryindex():
    if not auth.current_user().check_visibility(constants.SIDEBAR_CATEGORY):  # pyright: ignore[reportOptionalMemberAccess]
        abort(404)
    return render_element_index(db.Tags.name, db.books_tags_link, 'opds.feed_letter_category')  # pyright: ignore[reportGeneralTypeIssues]
...
def feed_letter_category(book_id):
    if not auth.current_user().check_visibility(constants.SIDEBAR_CATEGORY):  # pyright: ignore[reportOptionalMemberAccess]
        abort(404)
    off = request.args.get("offset") or 0
    letter = true() if book_id == "00" else func.upper(db.Tags.name).startswith(book_id)  # pyright: ignore[reportGeneralTypeIssues]
    entries = calibre_db.session.query(db.Tags)\
        .join(db.books_tags_link)\
        .join(db.Books)\
        .filter(calibre_db.common_filters()).filter(letter)\
        .group_by(text('books_tags_link.tag'))\
        .order_by(db.Tags.name)  # pyright: ignore[reportGeneralTypeIssues]
    pagination = Pagination((int(off) / (int(config.config_books_per_page)) + 1), config.config_books_per_page,
                            entries.count())
    entries = entries.offset(off).limit(config.config_books_per_page).all()  # pyright: ignore[reportArgumentType]
    cc = calibre_db.get_cc_columns(config, filter_config_custom_read=True)
...
def feed_seriesindex():
    if not auth.current_user().check_visibility(constants.SIDEBAR_SERIES):  # pyright: ignore[reportOptionalMemberAccess]
        abort(404)
    return render_element_index(db.Series.sort, db.books_series_link, 'opds.feed_letter_series')  # pyright: ignore[reportGeneralTypeIssues]
...
def feed_letter_series(book_id):
    if not auth.current_user().check_visibility(constants.SIDEBAR_SERIES):  # pyright: ignore[reportOptionalMemberAccess]
        abort(404)
    off = request.args.get("offset") or 0
    letter = true() if book_id == "00" else func.upper(db.Series.sort).startswith(book_id)  # pyright: ignore[reportGeneralTypeIssues]
    entries = calibre_db.session.query(db.Series)\
        .join(db.books_series_link)\
        .join(db.Books)\
        .filter(calibre_db.common_filters()).filter(letter)\
        .group_by(text('books_series_link.series'))\
        .order_by(db.Series.sort)  # pyright: ignore[reportGeneralTypeIssues]
    pagination = Pagination((int(off) / (int(config.config_books_per_page)) + 1), config.config_books_per_page,
                            entries.count())
    entries = entries.offset(off).limit(config.config_books_per_page).all()  # pyright: ignore[reportArgumentType]
    cc = calibre_db.get_cc_columns(config, filter_config_custom_read=True)
...
def feed_ratingindex():
    if not auth.current_user().check_visibility(constants.SIDEBAR_RATING):  # pyright: ignore[reportOptionalMemberAccess]
        abort(404)
    off = request.args.get("offset") or 0
    entries = (calibre_db.session.query(db.Ratings, func.count('books_ratings_link.book').label('count'),  # pyright: ignore[reportArgumentType]
        ...
        .order_by(db.Ratings.rating).all())  # pyright: ignore[reportGeneralTypeIssues]
...
def feed_formatindex():
    if not auth.current_user().check_visibility(constants.SIDEBAR_FORMAT):  # pyright: ignore[reportOptionalMemberAccess]
        abort(404)
    off = request.args.get("offset") or 0
    entries = (calibre_db.session.query(db.Data).join(db.Books)
        .filter(calibre_db.common_filters())
        .group_by(db.Data.format)  # pyright: ignore[reportGeneralTypeIssues]
        .order_by(db.Data.format).all())  # pyright: ignore[reportArgumentType,reportGeneralTypeIssues]
    pagination = Pagination((int(off) / (int(config.config_books_per_page)) + 1), config.config_books_per_page,
                            len(entries))
    element = list()
    for entry in entries:
...
def feed_languagesindex():
    if not auth.current_user().check_visibility(constants.SIDEBAR_LANGUAGE):  # pyright: ignore[reportOptionalMemberAccess]
        abort(404)
    off = request.args.get("offset") or 0
    if auth.current_user().filter_language() == "all":  # pyright: ignore[reportOptionalMemberAccess]
        languages = calibre_db.speaking_language()
    else:
        languages = calibre_db.session.query(db.Languages).filter(
            db.Languages.lang_code == auth.current_user().filter_language()).all()  # pyright: ignore[reportGeneralTypeIssues,reportOptionalMemberAccess]
...
@requires_basic_auth_if_no_ano
def feed_shelfindex():
    if not (auth.current_user().is_authenticated or g.allow_anonymous):  # pyright: ignore[reportOptionalMemberAccess]
        abort(404)
    off = request.args.get("offset") or 0
    shelf = ub.session.query(ub.Shelf).filter(
        or_(ub.Shelf.is_public == 1, ub.Shelf.user_id == auth.current_user().id)).order_by(ub.Shelf.name).all()  # pyright: ignore[reportOptionalMemberAccess]
    number = len(shelf)
    pagination = Pagination((int(off) / (int(config.config_books_per_page)) + 1), config.config_books_per_page,
                            number)
    cc = calibre_db.get_cc_columns(config, filter_config_custom_read=True)
...
def feed_shelf(book_id):
    if not (auth.current_user().is_authenticated or g.allow_anonymous):  # pyright: ignore[reportOptionalMemberAccess]
        abort(404)
    off = request.args.get("offset") or 0
    if auth.current_user().is_anonymous:  # pyright: ignore[reportOptionalMemberAccess]
        shelf = ub.session.query(ub.Shelf).filter(ub.Shelf.is_public == 1,
                                                  ub.Shelf.id == book_id).first()
    else:
        shelf = ub.session.query(ub.Shelf).filter(or_(and_(ub.Shelf.user_id == int(auth.current_user().id),  # pyright: ignore[reportOptionalMemberAccess]
                                                           ub.Shelf.id == book_id),
...
def opds_download_link(book_id, book_format):
    if not auth.current_user().role_download():  # pyright: ignore[reportOptionalMemberAccess]
        return abort(401)
    client = "kobo" if "Kobo" in request.headers.get('User-Agent') else ""  # pyright: ignore[reportOperatorIssue]
...
@requires_basic_auth_if_no_ano
def feed_read_books():
    if not (auth.current_user().check_visibility(constants.SIDEBAR_READ_AND_UNREAD) and not auth.current_user().is_anonymous):  # pyright: ignore[reportOptionalMemberAccess]
        return abort(403)
    off = request.args.get("offset") or 0
    result, pagination = render_read_books(int(off) / (int(config.config_books_per_page)) + 1, True, True)  # pyright: ignore[reportGeneralTypeIssues]
    cc = calibre_db.get_cc_columns(config, filter_config_custom_read=True)
    return render_xml_template('feed.xml', entries=result, pagination=pagination, cc=cc)
...
def feed_unread_books():
    if not (auth.current_user().check_visibility(constants.SIDEBAR_READ_AND_UNREAD) and not auth.current_user().is_anonymous):  # pyright: ignore[reportOptionalMemberAccess]
        return abort(403)
    off = request.args.get("offset") or 0
    result, pagination = render_read_books(int(off) / (int(config.config_books_per_page)) + 1, False, True)  # pyright: ignore[reportGeneralTypeIssues]
    cc = calibre_db.get_cc_columns(config, filter_config_custom_read=True)
    return render_xml_template('feed.xml', entries=result, pagination=pagination, cc=cc)
...
def render_xml_template(*args, **kwargs):
    ...
    xml = render_template(*args, current_time=currtime, instance=config.config_calibre_web_title, constants=constants.sidebar_settings, **kwargs)
...
def render_element_index(database_column, linked_table, folder):
    ...
    entries = calibre_db.session.query(func.upper(func.substr(database_column, 1, 1)).label('id'), None, None)  # pyright: ignore[reportCallIssue,reportArgumentType]
    # query = calibre_db.generate_linked_query(config.config_read_column, db.Books)
