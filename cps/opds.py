...
def feed_letter_author(book_id):
    ...
    return render_xml_template(
        "feed.xml", listelements=entries, folder="opds.feed_author", pagination=pagination, cc=cc
    )
...
def feed_publisherindex():
    ...
    cc = calibre_db.get_cc_columns(config, filter_config_custom_read=True)
    return render_xml_template(
        "feed.xml", listelements=entries, folder="opds.feed_publisher", pagination=pagination, cc=cc
    )
...
def feed_letter_category(book_id):
    ...
    cc = calibre_db.get_cc_columns(config, filter_config_custom_read=True)
    return render_xml_template(
        "feed.xml", listelements=entries, folder="opds.feed_category", pagination=pagination, cc=cc
    )
...
def feed_letter_series(book_id):
    ...
    cc = calibre_db.get_cc_columns(config, filter_config_custom_read=True)
    return render_xml_template(
        "feed.xml", listelements=entries, folder="opds.feed_series", pagination=pagination, cc=cc
    )
...
def feed_ratingindex():
    ...
    cc = calibre_db.get_cc_columns(config, filter_config_custom_read=True)
    return render_xml_template(
        "feed.xml", listelements=element, folder="opds.feed_ratings", pagination=pagination, cc=cc
    )
...
def feed_formatindex():
    ...
    cc = calibre_db.get_cc_columns(config, filter_config_custom_read=True)
    return render_xml_template(
        "feed.xml", listelements=element, folder="opds.feed_format", pagination=pagination, cc=cc
    )
...
def feed_languagesindex():
    ...
    cc = calibre_db.get_cc_columns(config, filter_config_custom_read=True)
    return render_xml_template(
        "feed.xml", listelements=languages, folder="opds.feed_languages", pagination=pagination, cc=cc
    )
...
def feed_shelfindex():
    ...
    shelf = (
        ...
        .filter(or_(ub.Shelf.is_public == 1, ub.Shelf.user_id == auth.current_user().id))
    ...
    if auth.current_user().is_anonymous:  # pyright: ignore[reportOptionalMemberAccess]
        ...
        shelf = (
            ...
            .filter(
                or_(
                    and_(
                        ub.Shelf.user_id == int(auth.current_user().id),  # pyright: ignore[reportOptionalMemberAccess]
                        ub.Shelf.id == book_id,
                    ),
...
def feed_read_books():
    if not (
        auth.current_user().check_visibility(constants.SIDEBAR_READ_AND_UNREAD) and not auth.current_user().is_anonymous
    ):  # pyright: ignore[reportOptionalMemberAccess]
        return abort(403)
    off = request.args.get("offset") or 0
