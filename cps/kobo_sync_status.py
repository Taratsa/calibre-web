...
def change_archived_books(book_id, state=None, message=None):
    ...
    if state is not None:
        archived_book.is_archived = state  # type: ignore[assignment]
    else:
        archived_book.is_archived = not archived_book.is_archived  # type: ignore[arg-type] # pyright: ignore[reportAttributeAccessIssue,reportGeneralTypeIssues]
    archived_book.last_modified = datetime.now(UTC)  # type: ignore[assignment]        # toDo. Check utc timestamp # pyright: ignore[reportAttributeAccessIssue]
...
def update_on_sync_shelfs(user_id):
    books_to_archive = (
        ...
        .filter(or_(ub.Shelf.kobo_sync == 0, ub.Shelf.kobo_sync.is_(None)))
