...
def getFolderInFolder(parentId, folderName, drive):
    ...
    folder = query + f"'{parentId}' in parents and mimeType = 'application/vnd.google-apps.folder' and trashed = false"
    fileList = drive.ListFile({"q": folder}).GetList()
...
def getFile(pathId, fileName, drive, nocase):
    metaDataFile = "'{}' in parents and trashed = false and title contains '{}'".format(
        pathId, fileName.replace("'", r"\'")
    )
...
def copyToDrive(drive, uploadFile, createRoot, replaceFiles, ignoreFiles=None, parent=None, prevDir=""):
    ...
    if os.path.isdir(os.path.join(prevDir, uploadFile)):
        ...
        ).GetList()  # pyright: ignore[reportOptionalSubscript]
...
def watchChange(drive, channel_id, channel_type, channel_address, channel_token=None, expiration=None):
...
def watchFile(drive, file_id, channel_id, channel_type, channel_address, channel_token=None, expiration=None):
...
def get_cover_via_gdrive(cover_path):
    ...
    if df:
        if not session.query(PermissionAdded).filter(PermissionAdded.gdrive_id == df["id"]).first():
            df.GetPermissions()
            df.InsertPermission({"type": "anyone", "value": "anyone", "role": "reader", "withLink": True})
