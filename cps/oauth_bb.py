    ...
    try:
        ...
        if oauth_entry is not None and oauth_entry.user:
            ...
            if current_user and current_user.is_authenticated and oauth_entry.user_id != current_user.id:
                flash(
                    _("This %(oauth)s account is already linked to a different user", oauth=provider_name),
                    category="error",
                )
                log.warning(
                    "User %s tried to link OAuth account already bound to user %s", current_user.id, oauth_entry.user_id
                )
...
def generate_oauth_blueprints():
    ...
    for element in oauthblueprints:
        ...
        element["blueprint"].backend = OAuthBackend(
            ub.OAuth,
            ub.session,
            str(element["id"]),  # pyright: ignore[reportPossiblyUnboundVariable]
            user=current_user,
            user_required=True,
        )
