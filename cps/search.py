...
def adv_search_custom_columns(cc, term, q):
    for c in cc:
        if c.datatype == "datetime":
            ...
            if custom_low:
                q = q.filter(
                    getattr(db.Books, "custom_column_" + str(c.id)).any(db.cc_classes[c.id].value >= custom_low)
                )
            if custom_high:
                q = q.filter(
                    getattr(db.Books, "custom_column_" + str(c.id)).any(db.cc_classes[c.id].value <= custom_high)
                )
        else:
            ...
            if c.datatype == "bool":
                if custom_query != "Any":
                    if custom_query == "":
                        q = q.filter(
                            ~getattr(db.Books, "custom_column_" + str(c.id)).any(db.cc_classes[c.id].value >= 0)
                        )
                    else:
                        q = q.filter(
                            getattr(db.Books, "custom_column_" + str(c.id)).any(
